...
Sphinx制御用「reST」 transifex の訳文に含まれるreST記法（前後の半角スペース含め）は後処理(Sphinx)で使用されるため翻訳しないこと。 ＜例１＞::で囲まれた文字列はロールを表し、テキストのスタイルを変更する。翻訳はしない。 原文 Some algorithms, even if they can produce a result with the given input data, might add comments or additional information to the :guilabel:Warning block if they detect potential problems with the data, in order to warn you.
